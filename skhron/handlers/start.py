...
def _help_text(is_admin: bool, bot_username: str | None) -> str:
    ...
    lines = [
        ...
        "(нужно, чтобы я был админом группы или был выключен privacy mode)",
        "/top — топ мемов группы по звёздочкам",
        "⭐️ под постами — мем попадает в твоё избранное (смотри в личке)",
