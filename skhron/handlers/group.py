...
async def group_save(
    ...
    /save ответом на сообщение с медиа.

    ВАЖНО: при включённом privacy mode Telegram НЕ передаёт боту
    reply_to_message на чужие сообщения (подтверждено на проде и
    официальным FAQ) — команда приходит «без реплая». Лечится статусом
    админа группы у бота или выключенным privacy mode (/setprivacy
    у BotFather + переустановка бота в группе)."""
    if message.reply_to_message is None:
        await message.reply(
            "Ответь командой /save на сообщение с мемом 🙂\n\n"
            "Если ты УЖЕ ответил, а я его не вижу — Telegram прячет от меня "
            "чужие сообщения (privacy mode). Чинится так: сделайте меня "
            "админом группы (заработает сразу), или владелец бота выключает "
            "privacy mode у BotFather (/setprivacy → Disable) и "
            "переДОБАВЛЯЕТ меня в группу."
        )
        return
    # бот мог попасть в группу до внедрения фичи — регистрируем лениво
    await repo.upsert_chat(
