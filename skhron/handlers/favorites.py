...
def _nav_row(offset: int, total: int) -> list[InlineKeyboardButton]:
    ...
    jump = FavPageCB(offset=-1).pack()
    prev_cb = FavPageCB(offset=offset - 1).pack() if offset > 0 else jump
    next_cb = FavPageCB(offset=offset + 1).pack() if offset + 1 < total else jump
    return [
